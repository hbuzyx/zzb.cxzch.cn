...
function setUserInfo(){
	...
	var xianqu=$("#xianqu").val();//取得职业信息
	var xiangzhen=$("#xiangzhen").val();//取得职业信息
	var shequ=$("#shequ").val();//取得职业信息
	...
	var ckemailreg=function(){
		...
		$.ajax({
	        ...
	        type: "POST",
	        url:REG_ADDRESS,
	        data:"uname="+uname+"&sex="+sex+"&password="+password+"&profession="+profession+"&xianqu="+xianqu+"&xiangzhen="+xiangzhen+"&shequ="+shequ+"&intro="+intro+udata+"&city_names="+city_names+"&city_ids="+city_ids+"&type="+type+"&verify="+verify,
	        dataType:"json",
	        success:function(data){
	            if(data.status=='0'){
	            	 notes(data.info,'failure');	
	...
	if(uname=="" || uname.length>6){//检查昵称
		...
		$.ajax({
	        ...
	        success:function(data){
	            if(data==0){
	            	 notes('对不起，此昵称格式不对或已被注册，请更换!','failure');	
	            	 return;
	            }else{
	            	ckemailreg();
	            }
