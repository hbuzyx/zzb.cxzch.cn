...
//$cid来判断
        ...
        $content = M($self['tablename'])->where(array('status' => 1, 'id' => $id))->find();
		...
		//上一页
		$front= D2('ArcView', $self['tablename'])->where(array($self['tablename'].'.status' => 1, 'cid' => $content['cid'], 'id' => array('lt',$content['id'])))->order('id desc')->find();
		if($front){
			$front['url'] = get_content_url($front['id'], $front['cid'], $front['ename']);
		}
		$this->assign('front', $front);
		//下一页
		$next= D2('ArcView',$self['tablename'])->where(array($self['tablename'].'.status' => 1, 'cid' => $content['cid'], 'id' => array('gt',$content['id'])))->order('id ASC')->find();
		if($next){
			$next['url'] = get_content_url($next['id'], $next['cid'], $next['ename']);
		}
          $content['contents']= mb_substr( strip_tags($content['content']), 0, 150);
	...
	public function viewpic(){
		...
		$content = M('soft')->where(array('status' => 1, 'id' => $id))->find();
        if (empty($content)) {
            $this->error('内容不存在');
        }
		$pictureurls_arr = empty($content['pictureurls']) ? array() : explode('|||', $content['pictureurls']);
		...
		$this->assign('title', $content['title'].'效果演示');
		$this->assign('content', $content);
        $this->display();
	}
    ...
    public function getpdemo(){
		$id = I('id', 0, 'intval');
		if ($id == 0) {
            $this->error('参数错误');
        }
		$demo = M('product')->where(array('status' => 1, 'id' => $id))->getField('demo');
		if(!$demo){
			$demo = '/';
		}
		redirect($demo);
