...
class VideoController extends CommonContentController
{
    ...
    public function index()
    {
        ...
        if ($pid) {
            ...
            //$where = array('Video.status' => 0, 'cid' => $pid);
            $where = array('Video.status' => array('gt',0), 'cid' => array('in', $idarr));
        } else {
            $where = array('Video.status' => array('gt',0));
        }

        if (!empty($keyword)) {
            $where['Video.title'] = array('LIKE', "%{$keyword}%");
        }
        ...
        $art   = D2('ArcView', 'Video')->nofield('content,pictureurls,updatelog,downlink')->where($where)->order('Video.id DESC')->limit($limit)->select();
    ...
    public function addPost()
    {
        ...
        if ($id = M('Video')->add($data)) {
            ...
            $this->success('添加成功', U('Video/index', array('pid' => $pid)));
        } else {
            $this->error('添加失败');
        }
    }
    ...
    public function editPost()
    {
        ...
        if (false !== M('Video')->save($data)) {
			...
			$p = I('p', 0, 'intval');
            $this->success('修改成功', U('Video/index', array('pid' => $pid,'p'=>$p)));
        } else {
    ...
    public function move()
    {
        ...
        if (IS_POST) {
            ...
            if (false !== M($actionName)->where(array('id' => array('in', $id)))->setField('cid', $cid)) {
                $this->success('移动成功', U('Video/index', array('pid' => $pid)));
            } else {
                $this->error('移动失败');
            }
            exit();
    ...
    public function trach()
    {
        ...
        $page->setConfig('theme', '%HEADER% %FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END%');
        $limit = $page->firstRow . ',' . $page->listRows;
        $art   = D2('ArcView', 'Video')->nofield('content,pictureurls,updatelog,downlink')->where($where)->limit($limit)->select();
    ...
    public function del()
    {
        ...
        if (false !== M('Video')->where(array('id' => $id))->setField('status', 0)) {

            del_cache_html('Show/index_*_' . $id . '.', false, 'show:index');
            $this->success('删除成功', U('Video/index', array('pid' => $pid)));
    ...
    public function delBatch()
    {
        ...
        if (false !== M('Video')->where(array('id' => array('in', $idArr)))->setField('status', 0)) {

            //更新静态缓存
            foreach ($idArr as $v) {
                del_cache_html('Show/index_*_' . $v . '.', false, 'show:index');
            }
            $this->success('批量删除成功', U('Video/index', array('pid' => $pid)));
    ...
    public function restore()
    {
        ...
        if (false !== M('Video')->where(array('id' => $id))->setField('status', 1)) {

            $this->success('还原成功', U('Video/trach', array('pid' => $pid)));
    ...
    public function restoreBatch()
    {
        ...
        if (false !== M('Video')->where(array('id' => array('in', $idArr)))->setField('status', 1)) {
    ...
    public function clear()
    {
        ...
        $pid     = I('get.pid', 0, 'intval');
        $modelid = D2('ArcView', 'Video')->where(array('id' => $id))->getField('modelid');
        ...
        if (M('Video')->delete($id)) {
            ...
            $this->success('彻底删除成功', U('Video/trach', array('pid' => $pid)));
        } else {
            $this->error('彻底删除失败');
        }
    }
    ...
    public function clearBatch()
    {
        ...
        $modelid = D2('ArcView', 'Video')->where(array('id' => $idArr[0]))->getField('modelid'); //
