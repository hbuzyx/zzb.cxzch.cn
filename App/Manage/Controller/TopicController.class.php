...
class TopicController extends CommonContentController
{
    ...
    public function index()
    {
        ...
        if ($pid) {
            ...
            $where = array('topic.status' => array('gt',0), 'cid' => array('in', $idarr));
        } else {
            $where = array('topic.status' => array('gt',0));
        }

        if (!empty($keyword)) {
            $where['topic.title'] = array('LIKE', "%{$keyword}%");
        }
        ...
        $art   = D2('ArcView', 'topic')->nofield('content')->where($where)->order('topic.id DESC')->limit($limit)->select();

        $this->assign('pid', $pid);
        $this->assign('keyword', $keyword);
        $this->assign('page', $page->show());
        $this->assign('vlist', $art);
        $this->assign('type', '话题列表');
    ...
    public function addPost()
    {
        ...
        if ($id = M('topic')->add($data)) {
            ...
            $this->success('添加话题成功', U('Topic/index', array('pid' => $pid)));
        } else {
            $this->error('添加话题失败');
        }
    }
    ...
    public function editPost()
    {
        ...
        if (false !== M('topic')->save($data)) {
			...
			$p = I('p', 0, 'intval');
            $this->success('修改成功', U('Topic/index', array('pid' => $pid,'p'=>$p)));
        } else {
    ...
    public function move()
    {
        ...
        if (IS_POST) {
            ...
            if (false !== M($actionName)->where(array('id' => array('in', $id)))->setField('cid', $cid)) {
                $this->success('移动成功', U('Topic/index', array('pid' => $pid)));
            } else {
                $this->error('移动失败');
            }
            exit();
    ...
    public function trach()
    {
        ...
        $page->setConfig('theme', '%HEADER% %FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END%');
        $limit = $page->firstRow . ',' . $page->listRows;
        $art   = D2('ArcView', 'topic')->nofield('content')->where($where)->limit($limit)->select();
    ...
    public function del()
    {
        ...
        if (false !== M('topic')->where(array('id' => $id))->setField('status', 0)) {

            del_cache_html('Show/index_*_' . $id . '.', false, 'show:index');
            $this->success('删除成功', U('Topic/index', array('pid' => $pid)));
    ...
    public function delBatch()
    {
        ...
        if (false !== M('topic')->where(array('id' => array('in', $idArr)))->setField('status', 0)) {
            ...
            //. M('topic')->getlastsql();
            $this->success('批量删除成功', U('Topic/index', array('pid' => $pid)));
    ...
    public function restore()
    {
        ...
        if (false !== M('topic')->where(array('id' => $id))->setField('status', 1)) {

            $this->success('还原成功', U('Topic/trach', array('pid' => $pid)));
    ...
    public function restoreBatch()
    {
        ...
        if (false !== M('topic')->where(array('id' => array('in', $idArr)))->setField('status', 1)) {
    ...
    public function clear()
    {
        ...
        $pid     = I('get.pid', 0, 'intval');
        $modelid = D2('ArcView', 'topic')->where(array('id' => $id))->getField('modelid');

        if (M('topic')->delete($id)) {
            // delete picture index
            if ($modelid) {
                M('attachmentindex')->where(array('arcid' => $id, 'modelid' => $modelid))->delete(); //test
            }
            $this->success('彻底删除成功', U('Topic/trach', array('pid' => $pid)));
        } else {
            $this->error('彻底删除失败');
        }
    }
    ...
    public function clearBatch()
    {
        ...
        $modelid = D2('ArcView', 'topic')->where(array('id' => $idArr[0]))->getField('modelid'); //
