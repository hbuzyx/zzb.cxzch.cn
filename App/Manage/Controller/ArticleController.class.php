...
class ArticleController extends CommonContentController
{
    ...
    public function index()
    {
        ...
        if ($pid) {
            ...
            $where = array('article.status' => array('gt',0), 'cid' => array('in', $idarr));
        } else {
            $where = array('article.status' => array('gt',0));
        }
    ...
    public function addPost()
    {
		...
		$data['video'] = I('video','','stripslashes');
    ...
    public function editPost()
    {
        ...
        if (false !== M('article')->save($data)) {
			...
			$p = I('p', 0, 'intval');
            $this->success('修改成功', U('Article/index', array('pid' => $pid,'p'=>$p)));
        } else {
