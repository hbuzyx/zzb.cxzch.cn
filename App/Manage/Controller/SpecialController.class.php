...
class SpecialController extends CommonController
{
    ...
    public function index()
    {
        ...
        $where   = array('special.status' => array('gt',0));
    ...
    public function add()
    {
        ...
        $_styleShowList = get_file_folder_List('./Tpl/Home/' . C('CFG_DEFAULT_TPL').'/Special', 2, 'Special_*');
    ...
    public function addPost()
    {
        ...
        if ($id = M('special')->add($data)) {
            ...
            $this->success('添加成功', U('Special/index', array('pid' => $pid)));
        } else {
            $this->error('添加失败');
        }
    }
    ...
    public function editPost()
    {
        ...
        if (false !== M('special')->save($data)) {
			...
			$p = I('p', 0, 'intval');
            $this->success('修改成功', U('Special/index',array('pid' => $pid,'p'=>$p)));
        } else {
