...
class Category
{
    ...
    //清除包含对应字段的分类
    public static function clearCate($cate, $status = 'status', $value = 1)
    {
        $arr = array();
        foreach ($cate as $v) {
            if ($v[$status] != $value) {
    ...
    public static function clearPageAndLink($cate, $modelid = 2, $type = 0)
    {
        $arr = array();
        foreach ($cate as $v) {
    ...
    //一维数组
    public static function toLevel($cate, $delimiter = '———', $pid = 0, $level = 0)
    {
        $arr = array();
        foreach ($cate as $v) {
            if ($v['pid'] == $pid) {
