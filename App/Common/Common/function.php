...
function get_category($status = 0, $update = 0)
{
    ...
    if ($update || !$cate_arr) {
        if ($status == 1) {
            $cate_arr = D('CategoryView')->nofield('content')->where(array('category.status' => 1))->order('category.sort asc,category.id')->select();
        } elseif ($status == 2) {
            //后台栏目专用
            $cate_arr = D('CategoryView')->nofield('content')->where(array('category.type' => 0))->order('category.sort asc,category.id')->select();
        } else {
            $cate_arr = D('CategoryView')->nofield('content')->order('category.sort asc,category.id')->select();
        }
        if (!isset($cate_arr)) {
            $cate_arr = array();
        }
...
{
    $user = D('MemberView')->nofield('password,encrypt')->order('id desc')->find($uid);
    return $user;
}
...
function getMoneyjinbi($money)
{
    $points = $money * C("JINBI");
...
function getTimes()
{
    ...
    for ($i = 0; $i <= 11; $i++) {
        $starttime = strtotime('-' . $i . ' month') - $t;
        $endtime = strtotime('-' . ($i - 1) . ' month') - $t;
        $times[$i]['title'] = date("m", $starttime) . "月";
        $times[$i]['starttime'] = $starttime;
        $times[$i]['endtime'] = $endtime;
        $times[$i]['start_date'] = date("Y-m-d", $starttime);
        $times[$i]['end_date'] = date("Y-m-d", $endtime);
    }
    return $times;
}
/**
