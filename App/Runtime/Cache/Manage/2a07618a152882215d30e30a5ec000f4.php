...
<head>
	...
	<script language="JavaScript">
	    <!--
	    var URL = '/xn_admin.php?s=/Tag';
	    var APP	 = '/xn_admin.php?s=';
	    var SELF='/xn_admin.php?s=/Tag/index';
    ...
    <div class="row margin-botton">
        <div class="col-sm-6 column">
            <div class="btn-group btn-group-md">
                <button class="btn btn-primary" type="button" onclick="goUrl('<?php echo U('add');?>')"><em class="glyphicon glyphicon-plus-sign"></em> 添加标签</button>
                ...
                <div class="table-responsive">
                    <table class="table table-hover xyh-table-bordered-out">
                        ...
                        <?php if(is_array($vlist)): foreach($vlist as $key=>$v): ?><tr>
                            <td><input type="checkbox" name="key[]" value="<?php echo ($v["tag_id"]); ?>"></td>
                            <td><?php echo ($v["tag_id"]); ?></td>
                            <td><?php echo ($v["tablename"]); ?></td>
                            <td><?php echo (get_catename($v["typeid"])); ?></td>
                            <td><?php echo ($v["tag_name"]); ?></td>
                            <td><?php echo ($v["tag_sort"]); ?></td>
                            <td><?php echo (date('Y-m-d H:i:s', $v["posttime"])); ?></td>
                            <td class="text-right">
                                <a href="<?php echo U('edit',array('tag_id' => $v['tag_id']), '');?>" class="label label-success">编辑</a>
                                <a href="javascript:;" onclick="toConfirm('<?php echo U('del',array('tag_id' => $v['tag_id']), '');?>', '确实要删除吗？')" class="label label-danger">删除</a>
